...
#
# Exemplo:
#  StringFix.nome_proprio('maria de souza dos santos e silva da costa') ==> 'Maria de Souza dos Santos e Silva da Costa'
...
#  texto ==> 'Maria de Souza dos Santos e Silva da Costa'
def nome_proprio!
self.gsub!(/^.*$/, StringFix.nome_proprio(self)) if self
end
# Remove as letras acentuadas
#
# Exemplo:
#  StringFix.remover_acentos('texto está com acentuação') ==> 'texto esta com acentuacao'
...
def remover_acentos
StringFix.remover_acentos(self)
end
...
def remover_acentos!
self.gsub!(/^.*$/, StringFix.remover_acentos(self)) if self
end
# Retorna uma stringFix com caracteres maiusculos
#
# Exemplo:
#  StringFix.upcase('texto com acentuação' ==> 'TEXTO COM ACENTUAÇÃO'
...
#  texto ==> 'TEXTO COM ACENTUAÇÃO'
def upcase!
self.gsub!(/^.*$/, StringFix.upcase(self)) if self
end
# Retorna uma stringFix com caracteres minúsculos
#
# Exemplo:
#  StringFix.downcase('TEXTO COM ACENTUAÇÃO') ==> 'texto com acentuação'
...
#  texto ==> 'texto com acentuação'
def downcase!
self.gsub!(/^.*$/, StringFix.downcase(self)) if self
end
# Passa a primeira letra de cada palavra para maiúscula e as demais para minúsculas.
#
# Exemplo:
#  StringFix.titleize('o livro esta sobre a mesa!') ==> 'O Livro Esta Sobre A Mesa!'
def self.titleize(texto)
return texto if texto.nil? or texto.empty?
    texto = texto.downcase
    texto = texto.downcase
    texto.mb_chars[0] = texto.mb_chars.first.upcase
    texto = texto.gsub(/\s[a-z#{StringFix::MINUSCULAS_COM_ACENTO}]/) {|a| a.upcase }
    texto
end
# Passa a primeira letra de cada palavra para maiúscula e as demais para minúsculas.
#
...
#  texto ==> 'O Livro Esta Sobre A Mesa!'
def titleize!
self.gsub!(/^.*$/, StringFix.titleize(self)) if self
end
end